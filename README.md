...
현재는 SSL 안정성을 위해 재생목록을 순차 처리합니다. `--workers` 옵션은 제거되었습니다.
...
### 2. 순차 처리 안정화
- `PlaylistExtractor`는 SSL 연결 안정성을 위해 재생목록을 순차 처리
- 불안정한 병렬 호출 대신 실패율을 줄이는 운영 방식을 기본값으로 사용
